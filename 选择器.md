...
如果*value*属性被指定，不论是否指定类都会与选定元素相结合。如果*value*是一个常量，那么所有的元素都会被分配指定的类（还没分配的话）。如果不是那么就会移除选中元素的class（已经分配过）。如果*value*是一个函数，那么这个函数会为每个选中的元素（按顺序）计算。入参是当前数据元素`d`和当前索引`i`，以及代表当前DOM元素的`this`上下文。这个函数的返回值接下来用来分配或者不分配每个元素的class。
